...
`BehaviorSignature` is the v0.2 behavior layer built from the normalized trace
events above. It is a separate schema from `trace.Event`: the trace remains the
adapter-independent fact log, while behavior signatures summarize those facts
into comparable behavior steps.

The behavior signature schema is emitted by `agent-vcr behavior ... --json` and
uses its own schema version. It contains:
...
Each behavior step is agent-independent and may use kinds such as:
...
Behavior signatures are used by `agent-vcr behavior diff` to report first
behavior divergence. The existing `agent-vcr diff` command remains the
event-level diff over the normalized trace schema.
